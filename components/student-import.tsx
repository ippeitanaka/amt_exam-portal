  ...
  const handleFileUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        try {
          // ローカルストレージから既存の学生データを取得
          const cachedStudentsStr = localStorage.getItem("cachedStudents")
          const cachedStudents = cachedStudentsStr ? JSON.parse(cachedStudentsStr) : []

          // 学生データをマージするためのマップを作成
          const studentMap = new Map()

          // 既存のデータをマップに追加
          cachedStudents.forEach((student: any) => {
          ...
          for (const student of students) {
            studentMap.set(student.student_id, {
              ...student,
              // 既存のデータがある場合は、一部のフィールドを保持
              ...(studentMap.has(student.student_id)
                ? {
                    created_at: studentMap.get(student.student_id).created_at,
                  }
                : {}),
            })
          }
          ...
          setSuccess(`${students.length}件の学生情報をインポートしました`)

          toast({
            title: "インポート完了",
            description: `${students.length}件の学生情報をインポートしました`,
          })

          // インポート成功をコンポーネントに通知
          if (onImportSuccess) {
            onImportSuccess(updatedStudents)
          }
        ...
        <CardContent className="space-y-4 pt-6 bg-white dark:bg-brown-900">
          ...
          <div className="space-y-2">
            ...
            <Alert className="bg-yellow-50 border-yellow-200 text-yellow-800 dark:bg-yellow-900 dark:border-yellow-800 dark:text-yellow-100">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>
                現在、インポートした学生情報はローカルストレージに保存されます。
                ブラウザのデータをクリアすると、インポートした学生情報が失われる可能性があります。
              </AlertDescription>
            </Alert>
          </div>
